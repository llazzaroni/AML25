...
def main() -> None:
    ...
    for top_k_corr in spaced_ints(205, 205, 1):
        for rf_keep in spaced_ints(180, 180, 1):
            ...
            pipe = Pipeline([
                ("model", lgb.LGBMRegressor(
                    n_estimators=3000,
                    learning_rate=0.03,
                    num_leaves=127,          # 63/127/255 are good anchors
                    min_child_samples=10,    # 5–30
                    subsample=0.9, subsample_freq=1,     # bagging
                    colsample_bytree=0.9,                # feature subsampling
                    reg_alpha=0.0, reg_lambda=1.0,       # L1/L2
                    random_state=SEED,
                    n_jobs=-1,
                    verbosity=-1
                ))
